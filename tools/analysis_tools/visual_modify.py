...
class Viusal_tool():
    def __init__(self, pred_data, sample_token_list, out_folder, version):
    ...
    def render_sample_data(
        ...
        """
        sample_toekn = self.sample_token_list[id]
        pred_data = self.pred_data
        out_path = os.path.join(self.out_folder, self.sample_token_list[id])
        self.lidiar_render(sample_toekn, pred_data, out_path=out_path)
        ...
        for ind, cam in enumerate(cams):
            ...
            if sensor_modality in ['lidar', 'radar']:
                ...
                img_path = os.path.join(out_path+'_'+cam+'.png')
                data.save(img_path) 
...
if __name__ == '__main__':
    ...
    # bevformer_results = mmcv.load('test/bevformer_small/Mon_Dec_25_04_44_35_2023/pts_bbox/results_nusc.json')
    bevformer_results = mmcv.load('test/bevformer_small/origin_trained/pts_bbox/results_nusc.json')
    bevformer_version = 'bevformer_small_origin_trained'
    sample_token_list = list(bevformer_results['results'].keys())
    out_folder = os.path.join('outputs',bevformer_version)
